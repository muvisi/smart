...
class SyncMemberRenewalsView(APIView):
    """
    Sync corporate member renewals (category changes) from HAIS to SMART
    """
    ...
    def get_hais_member_renewals(self, hais_token):
        payload = {"name": "smartCorporateMemberRenewals", "param": {}}
        resp = requests.post(
            settings.HAIS_API_BASE_URL,
            json=payload,
            headers={
    ...
    def create_hais_log(self, hais_token, smart_httpcode, request_obj, response_obj):
        payload = {
            ...
            "param": {
                "source": "HAIS-SMART",
                "transactionName": "Member Renewal",
    ...
    def post(self, request):
        ...
        members = member_renewals_resp["response"]["result"]
        success, failed = 0, 0

        for m in members:
            category = f"{m.get('category')}-{m.get('anniv')}"
            payload = {
                "memberNumber": m.get("member_no"),
                "newGrade": category,
            ...
            smart_url = f"{settings.SMART_API_BASE_URL}members/categorychange?{urlencode(payload)}"
            ...
            # update HAIS status
            self.update_hais_member_status(
                hais_token, m.get("member_no"), m.get("anniv"), sync_status
            )
            # log transaction
        ...
        return Response({
            "response": {
                "result": f"{success} member renewal(s) successfully synced to SMART, {failed} failed"
            }
        })
