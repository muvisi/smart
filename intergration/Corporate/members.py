...
class SmartMemberSyncService:
    ...
    def _get_smart_token(self):
        ...
        try:
            ...
            logger.error(f"SMART Member Auth Failure: {str(e)}")
            return None

    def run_member_sync(self):
        """Fetches unsynced members and pushes them to SMART."""
        sync_stats = {"success": 0, "failed": 0, "total": 0}
        
        try:
            with connections[self.mssql_alias].cursor() as mssql_cursor:
                # 1. Fetch data in JSON-like structure (TOP 25)
                query = "SELECT TOP 25 * FROM dbo.smart_corp_members_new"
                mssql_cursor.execute(query)
                columns = [col[0] for col in mssql_cursor.description]
                rows = mssql_cursor.fetchall()
            ...
            logger.error(f"Member Sync MSSQL Error: {str(e)}")
            return {"status": "error", "message": "External database unavailable."}

    def _process_member(self, mssql_cursor, val):
        """Handles Name/Phone parsing, JSON API call, and Atomic Audit."""
        ...
        smart_payload = {
            'familyCode': str(val.get('family_no')),
            'membershipNumber': str(val.get('member_no')),
            'staffNumber': str(val.get('member_no')),
            'surname': surname,
            'secondName': second_name,
            'thirdName': third_name,
            'dob': str(val.get('dob')) if val.get('dob') else None,
            'gender': val.get('gender') or "",
            'nhifNumber': val.get('nhif_no') or "",
            'memType': val.get('member_type'),
            'schemeStartDate': str(val.get('start_date')),
            'schemeEndDate': str(val.get('end_date')),
            'clnCatCode': f"{val.get('category')}-{val.get('anniv')}",
        ...
        status_code = 500
        try:
            api_url = f"{settings.SMART_API_BASE_URL}members"
            res = requests.post(
                api_url, 
                json=smart_payload, 
                headers={"Authorization": f"Bearer {self.smart_token}"}, 
        ...
        # 3. Atomic Database Update and Audit Trail
        try:
            with transaction.atomic(using=self.audit_db):
                sync_status = 1 if is_ok else 2
            ...
            logger.critical(f"Atomic Rollback for Member {val.get('member_no')}: {str(e)}")
            return False