...
class SmartMemberSyncService:
    ...
    def run_member_sync(self):
        ...
        try:
            with connections[self.mssql_alias].cursor() as mssql_cursor:
                # 1. Try first table
                mssql_cursor.execute("SELECT TOP 100 * FROM dbo.smart_corp_members_new")
                columns = [col[0] for col in mssql_cursor.description]
                rows = mssql_cursor.fetchall()

                # 2. If first table empty, try renewals
                if not rows:
                    mssql_cursor.execute("SELECT TOP 100 * FROM dbo.smart_corp_member_renewals")
                    columns = [col[0] for col in mssql_cursor.description]
                    rows = mssql_cursor.fetchall()
